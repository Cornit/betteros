-- BetterOS updater: stage 2

local fs = rawget(_G, "fs")
local term = rawget(_G, "term")
local http = rawget(_G, "http")

_G._BETTEROS_ROM_DIR = _BETTEROS_ROM_DIR or "/betteros"
if _BETTEROS_ROM_DIR == "/rom" then _BETTEROS_ROM_DIR = "/betteros" end

-- fail-safe
local start_betteros = [[
...
local title = "BetterOS Updater (Failure Notice)"
...
local message = {
  "A BetterOS update has failed or",
...
pcall(fs.delete, _BETTEROS_ROM_DIR)
pcall(fs.delete, "/.start_betteros.lua")
...
local handle = fs.open("/.start_betteros.lua", "w")
handle.write(start_betteros)
handle.close()
...
local function header()
  ...
  at(1, 1).clearLine()
  at(1, 1).write("BetterOS Updater (Stage 2)")
  at(1, 2).clearLine()
  at(1, 2).write("===========================")
  term.setTextColor(0x1)
end
...
local repodata = json.decode(dl("https://api.github.com/repos/cornit/betteros/git/trees/main?recursive=1"))
...
for _, v in pairs(repodata.tree) do
  if v.path and v.path:sub(1,#look) == look then
    ...
    v.real_path = v.path:gsub("^/?rom", _BETTEROS_ROM_DIR)
    to_dl[#to_dl+1] = v
  end
end
...
for i=1, #to_dl do
  ...
  if v.type == "blob" and v.real_path ~= "unbios.lua" then
    local data = dl("https://raw.githubusercontent.com/cornit/betteros/main/data/computercraft/lua/"..v.path)
    write(v.real_path)
    progress(i, #to_dl)
    if v.real_path == "bios.lua" then
      v.real_path = "/.start_betteros.lua"
    end
