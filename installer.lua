--[[
Original Author: Ocawesome101
Modified by: Cornit
]]

-- BetterOS installer

local DEFAULT_ROM_DIR = "/betteros"
...
-- set up package.loaded for BetterOS libs
package.loaded.betteros = {
...
package.loaded["betteros.json"] = json

local function rcload(f)
  return ghload(
    "cornit/betteros/main/data/computercraft/lua/rom/"..f, f)
end
...
settings.set("betteros.rom_dir", ROM_DIR)
settings.save()

tu.coloredPrint(colors.white, "Installing BetterOS to ", colors.lightBlue,
...
assert(io.open(
 fs.exists("/startup.lua") and "/unbios-betteros.lua" or "/startup.lua", "w"))
  :write(dl(
   "https://raw.githubusercontent.com/cornit/betteros/main/unbios.lua"
  )):close()
