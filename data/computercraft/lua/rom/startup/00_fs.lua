...
-- path resolution:
-- if the path begins with /betteros, then redirect to wherever that actually
...
local function resolve(path)
  ...
  if segments[1] == "betteros" then
    return fs.combine(_BETTEROS_ROM_DIR, table.concat(segments, "/", 2, #segments))
  else
    return path
  end
end
...
function fs.list(path)
  ...
  if path == "/" then
    -- inject /betteros into the root listing
    if not exists("/betteros") then
      files[#files+1] = "betteros"
    end
  end
  return files
end
