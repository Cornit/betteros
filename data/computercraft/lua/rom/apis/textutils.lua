...
local json = require("betteros.json")
  ...
  strings.wrappedWriteElements(elements, w, false, {
    newline = function()
      ...
      if lines >= h - 2 then
        ...
        betteros.write("Press any key to continue")
        term.setTextColor(old)
        betteros.pullEvent("char")
...
local function coloredWrite(paged, ...)
  ...
  local write = paged and pagedWrite or betteros.write
    ...
    if type(lini) == "number" then
      ...
      if written + max_len > w then
        if written + len > w then
          prt("\n")
          prt(lini)
          betteros.write((" "):rep(max_len - len))
          written = max_len
...
function tu.coloredPrint(...)
  return coloredWrite(false, ...) + betteros.write("\n")
end

function tu.coloredPagedPrint(...)
  return coloredWrite(true, ...) + betteros.write("\n")
end
